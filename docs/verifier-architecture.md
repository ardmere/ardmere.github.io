...
> Minimal by design — **the contract has no state, no upgrade path, no external storage references**; it only emits events. Frontend queries history via `eth_getLogs`; each `(exchange, snapshotId)` has **exactly one** `SnapshotAnchored` record (1 tx = 1 period). Neither `artifactBundleRoot` nor `verificationBundleRoot` may be zero.

**Deployed instance (Base Sepolia testnet)** — see [`deployments.md`](./deployments.md):
