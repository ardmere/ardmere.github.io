...
func (a *Adapter) FetchCurrentSnapshot(ctx context.Context, outDir string, opts exchange.FetchOpts) (exchange.FetchResult, error) {
	...
	} else if opts.AuditID != "" {
		parsed, summaryBytes, err = okxapi.FetchSummaryByAuditID(ctx, opts.AuditID)
		if err != nil {
			return exchange.FetchResult{}, fmt.Errorf("fetch okx audit %s: %w", opts.AuditID, err)
		}
	} else {
